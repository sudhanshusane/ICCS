...
%
One of the features of interest is the distribution of high-density clusters.
%
...
%
We 
%operated on the domain below the surface layer and 
...
%\textbf{Cloverleaf3D:} We included a mini-application for a benchmarking study. The Cloverleaf3D ECP proxy simulation code~\cite{mallinson2013cloverleaf} solves compressible Euler equations in a hydrodynamics setting on a Cartesian grid using explicit second-order method.
%
%Cloverleaf3D has been developed and used by several studies to evaluate emerging architectures and various techniques targeting Exascale applications.
...
For each application in this study, we organized our experiments to inform in situ encumbrance and post hoc efficacy. 
%
...
%For EC3, we limited our measurement and discussion of I/O to the total data storage  required on the file system and report it in Bytes stored.
...
%We used the post hoc efficacy of various Eulerian configurations storing full-resolution meshes at varying storage intervals as a reference for comparison.
%
%To measure the pathline reconstruction error, we calculated the average Euclidean distance of interpolated points from the ground truth~(precomputed using the complete simulation data) for each trajectory. 
%
%We visualized distributions of pathline error and reconstructed fields using violin plots with included boxplots to capture interquartile ranges.
%
%Our qualitative comparisons for both applications involved visualizing reconstructed time-varying vector fields using pathlines or derived scalar fields and observing the preservation of features of interest.
%
%
